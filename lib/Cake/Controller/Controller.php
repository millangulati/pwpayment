...
/**
 * Holds HTTP Request methods to be accepted.
 * Useful in API
 *
 * @var array
 */
...
/**
 * Holds HTTP Response Beheviour Corrosponding to Extension Passed or HTTP_Accept vairable.
 * Useful in API
 *
* @var false|rest|soap|<userdefined>
 */
...
/**
 * Parse after beforerender api Response
 *
 * @param mixed $response Response from after beforerender callback
 * @return void
 */
	protected function _render() {
		if($this->response->statusCode() != 200){
			$this->response->data = array('error' => true, 'code' => $this->response->statusCode(), 'message' => $this->viewVars['message']);
			$this->httpApi = $this->request->param('api');
			$this->viewClass = $this->request->param('vc');
		} else if ($this->request->param('api') == 'JwtRest'){
			try {
				App::uses('jwt', 'Utility');
				$this->response->data = array_merge(array('error' => false, 'code' => 200, 'message' => ''), (array)$this->response->data);
				$token = jwt::encode(array_diff_key((array)$this->response->data, array_flip(array('error', 'code', 'message'))), $this->jwt_key, $this->jwt_alg);
				$this->response->data = array('error' => $this->response->data['error'], 'code' => $this->response->data['code'], 'message' => $this->response->data['message'], 'token' => $token);
			} catch( Exception $ex) {
				throw (Configure::read('debug') > 0 ? $ex :  new InternalErrorException);
			}
		} else {
			$this->response->data = array_merge(array('error' => false, 'code' => 200, 'message' => ''), (array)$this->response->data);
		}
 ...
 */
	public function render($view = null, $layout = null) {
		...
		$this->getEventManager()->dispatch($event);
		if($this->request->param('api')){
			$m = array();
			try {
				$this->_render();
			} catch( Exception $e) {
				$m = array (get_class($e), $e->getMessage());
			}
			if(!empty($m)) {
				throw new $m[0]($m[1]);
			}
